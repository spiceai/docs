...
The same split applies to search. Build full-text and vector indexes on the cluster. Sidecars serve repeated searches from the [search results cache](../../features/caching) instead of rebuilding those indexes per pod. See [Search](../../features/search).
