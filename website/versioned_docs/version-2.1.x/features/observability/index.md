...
The three cache families (`results_cache_*`, `search_results_cache_*`, `embeddings_cache_*`) register the **same** instrument set, so every cache name below appears under all three prefixes in a scrape. The stale-serving counters — `*_cache_stale_rejections`, `*_cache_stale_swr_count` and `*_cache_swr_background_query_count` — are only ever incremented by the SQL results cache; their `search_results_cache_*` and `embeddings_cache_*` copies stay at zero. Read them on the `results_cache_*` prefix only.
