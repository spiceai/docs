...
| `cayenne_metadata_dir`            | Custom directory for storing Cayenne metadata (SQLite catalog). When unset, a local `cayenne_file_path` selects `{cayenne_file_path}/metadata`; otherwise the directory is `{spice_data_path}/metadata`, including when `cayenne_file_path` is an object-store URL. |
