...
Elasticsearch requests participate in [task history](../../../reference/task_history) through the HTTP client's span. Each `_search` and `_mapping` call is a child of the enclosing `sql_query` or `acceleration_refresh` task.
