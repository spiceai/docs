...
The MongoDB data connector can be configured by providing the following `params`. Use the [secret replacement syntax](/docs/components/secret-stores) to load the secret from a secret store, e.g. `${secrets:my_mongodb_conn_string}`.
