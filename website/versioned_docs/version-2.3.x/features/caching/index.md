...
A dataset **reload** counts as a change on that clock from v2.3.2: a hot reload, or any other re-registration of a dataset, invalidates the results cached from its previous contents, both when the reload starts and again once the new registration is in place, so a query that began mid-reload cannot store a result the clock would then accept. Before v2.3.2 a reload cleared only the [logical plan cache](#logical-plan-cache), and results read from the dataset's previous contents kept being served as fresh until `item_ttl` expired. If the invalidation cannot be recorded the reload still completes, and the runtime warns that queries may be answered from the previous contents until they expire.
