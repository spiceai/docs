...
| ---------------------------------------------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `Invalid connection pool configuration: connection_pool_min_idle (20) cannot be greater than connection_pool_max (10)` at dataset load | `pg_connection_pool_min` is larger than `connection_pool_size`; the two numbers in the message are the configured values. | Correct the values so `pg_connection_pool_min <= connection_pool_size`.                             |
