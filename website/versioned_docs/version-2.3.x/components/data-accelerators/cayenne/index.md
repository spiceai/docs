...
Cayenne caches the scan input a query reads — the merged view of files, in-memory rows, and deletions — and shares one build across concurrent scans. How long a cached view is reused depends on the dataset, and from v2.3.2 it is decided per dataset rather than on a timer everywhere:
...
Secondary indexes are available from v2.3.2. On v2.3.1 and earlier, `indexes` is ignored on a Cayenne acceleration.
