...
Cayenne prunes without any explicit index. Each Vortex segment carries `min`, `max`, `null_count`, `is_sorted`, and `is_constant` statistics per column, and a query prunes every segment whose range cannot match its predicate before reading it.
...
From v2.3.2 the persisted `min`/`max` bounds also cover decimal, binary, `Time32`/`Time64`, `Float16` and dictionary-encoded columns; earlier versions dropped them on the way into the statistics blob, so a predicate on such a column pruned nothing and a decimal clustering key collapsed onto its sign bit. Widening a decimal column's **scale** drops the persisted bounds for that table, which are rebuilt from the file footers — until then pruning is conservative, reading files it could have skipped, never skipping ones it should read.

How much a query prunes depends on the physical order of the data:
...
- **A `refresh_mode: full` table is sorted by the replace itself, from v2.3.1.** `sort_columns` otherwise takes effect only in the compaction rewrite, and a whole-table replace leaves nothing to consolidate, so compaction never runs on such a table. From v2.3.1 Cayenne orders the replacement stream before writing the new snapshot — the only write a full-refresh table makes, and so the only chance to establish the order. On v2.3.0 the replacement keeps arrival order. This applies to an **operator-configured** `sort_columns` only; an inference-derived order (`cayenne_sort_columns_origin: inferred`) does not trigger it.
- **A full refresh that configures no order still gets one, from v2.3.2.** A whole-table replace used to hash its rows across the write's shards, so every file — and, unless the source returned rows in key order, every zone inside it — spanned the whole key domain and a lookup on the key opened all of them. The replace now routes rows into **key-range files**: it samples up to 65,536 values of one column from the snapshot it is replacing, cuts them into equal-count ranges, one per output file, and sorts the rows inside each range in runs of up to 128 MiB. The column is [`cayenne_shard_key_columns`](./index.md#acceleration-parameters-accelerationparams) when set, otherwise the leading primary-key column — unless scans filter on some other column more than twice as often, which is the common case when schema inference supplied an auto-increment surrogate key that no lookup uses. A table with no primary key routes on its most-filtered column. It needs no configuration, the file count is unchanged, and no ordering is advertised to the planner: the layout only narrows what a scan has to open.

  It does not apply to a table that already configures `sort_columns` or `cayenne_cluster_by` (those keep their single sorted writer), to the first load of an empty table (there is no snapshot to sample), or to the children of a partitioned table (one writer each). A dictionary-encoded key column routes by range but skips the in-range sort. The rows and sort scratch are charged to the query memory pool; when the pool refuses, the run is sealed early and the batch is written as it arrived, so memory pressure costs sort order and never the write.
