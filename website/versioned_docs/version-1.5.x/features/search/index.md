...
Configuring embeddings is required for vector-based search. For detailed instructions on setting up embeddings, refer to [Configured Embeddings](../components/embeddings).

For performing vector-based search, see [Vector-Based Search](search/vector-search).
...
For performing full text search, see [Full text Search](search/full-text).
...
For an entire specification of the `vector_search` UDTF, see [Vector-Based Search](search/vector-search#sql-udtf).
...
For an entire specification of the `text_search` UDTF, see [Full text Search](search/full-text#sql-udtf).
