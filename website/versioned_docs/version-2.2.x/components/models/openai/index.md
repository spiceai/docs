...
| `openai_responses_tools`  | Comma-separated list of OpenAI-hosted tools exposed via the Responses API for this model.  These hosted tools are **not** available from the `/v1/chat/completions` HTTP endpoint. Supported tools: `code_interpreter`, `web_search`, `web_search_preview`.                                    | -                           |

`web_search` selects OpenAI's current web search tool. `web_search_preview` selects the legacy tool of the same name, retained for older models that do not support the current one — prefer `web_search` unless the model rejects it.
