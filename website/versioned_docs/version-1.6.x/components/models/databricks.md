...
| `databricks_token`         | The Databricks API token to authenticate with the Databricks Models API. Use the [secret replacement syntax](../secret-stores/index) to reference a secret, e.g., `${secrets:my_databricks_token}`. |
