...
| `s3_vectors_aws_iam_role_source`   | Optional. IAM role credential source: `auto` (default AWS credential chain), `metadata` (instance/container metadata only), or `env` (environment variables only). Ignored when an access key and secret are supplied. Default `auto`. | `metadata` |
...
- [Spice.ai announcement](https://spice.ai/blog/getting-started-with-amazon-s3-vectors-and-spice)
