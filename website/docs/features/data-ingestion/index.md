...
Data point attributes become additional columns named after the attribute key, typed by the attribute value (`Utf8`, `Boolean`, `Int64`, `Float64`, or `Binary`). The **resource** attributes of the export — `service.name`, `service.instance.id`, and anything else the sender sets on the resource — are merged into every data point in that group and become columns the same way, so points from two processes reporting the same metric stay distinguishable. When a key is carried by both, the data point's own value wins: it describes that measurement more specifically than the process-level one. When a metric starts reporting a new attribute, Spice evolves an accelerated table's schema in place before writing, subject to the dataset's [`on_schema_change`](../../reference/spicepod/datasets.md#on_schema_change) policy.
