...
Results caching improves performance for repeated requests and non-accelerated results, such as refresh data returned [on zero results](../data-acceleration/data-refresh.md#behavior-on-zero-results).
...
- **Caching accelerator SWR**: Configure `acceleration.params.caching_stale_while_revalidate_ttl` for [HTTP-based dataset caching](../data-acceleration/refresh-modes/caching.md)
