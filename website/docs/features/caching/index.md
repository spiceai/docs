...
Results caching improves performance for repeated requests and non-accelerated results, such as refresh data returned [on zero results](/docs/features/data-acceleration/data-refresh#behavior-on-zero-results).
...
Cache metrics can be monitored using the [Prometheus-compatible Metrics Endpoint](/docs/features/observability/index). The following metrics are available for each cache type:
