...
| `results_cache_stale_rejections`<br/><br/>Number of lookups that found an entry but refused to serve it because a table the result read had since been invalidated. Split by `reason` into `no_window` (no `stale_while_revalidate_ttl` configured), `window_expired` (the window had closed), or `fresh_required` (the lookup was made on a path that serves only fresh results). These are also counted as misses. | _count_ | `reason` |
