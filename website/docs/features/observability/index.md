...
The cache counters (`results_cache_*`, `search_results_cache_*`, `embeddings_cache_*`) are published at zero when the runtime starts, so a counter that has not yet fired appears in a scrape as a zero series rather than being absent — an absent series indicates a scrape or exporter problem, not an idle cache.

All three families register the **same** instrument set, so every name below appears under all three prefixes in a scrape. The stale-serving and invalidation counters — `*_cache_stale_rejections`, `*_cache_table_invalidations`, `*_cache_swr_revalidations`, `*_cache_invalidation_stale_hits`, `*_cache_stale_swr_count` and `*_cache_swr_background_query_count` — are only ever incremented by the SQL results cache; their `search_results_cache_*` and `embeddings_cache_*` copies are exported at zero and stay there. Read them on the `results_cache_*` prefix only.

Component-specific instruments are documented with their component. The `cayenne_*` families — CDC apply, write and compaction, maintenance, storage footprint, metastore, primary-key index, and memory attribution — are listed in the [Cayenne deployment guide](../components/data-accelerators/cayenne/deployment#metrics).
...
| `catalog_acceleration_accelerated_tables`<br/><br/>Accelerated tables in a CDC-accelerated catalog, labeled by catalog and acceleration kind (primary_key, unique_index, full). | _gauge_ | `catalog`, `kind` |
| `catalog_acceleration_tables`<br/><br/>Relations resolved by a CDC-accelerated catalog, labeled by catalog and category (accelerated, skipped, excluded, views_not_replicated). | _gauge_ | `catalog`, `category` |
...
| `dataset_acceleration_refresh_errors`<br/><br/>Number of errors refreshing the dataset. Always split by `reason`: `object_generation_changed` (a listed object was replaced while the scan still held the previous generation — a retriable precondition failure, not corruption), `parquet_decode` (the Parquet payload itself failed to decode), or `other`. | _count_ | `dataset`, `mode`, `reason` |
...
| `dataset_acceleration_write_back_pending_keys`<br/><br/>Committed keys awaiting [durable write-back](../components/data-accelerators/cayenne/#transactions). Updated on every delivery pass, including failures; resets to `0` when the dataset unloads. A persistent backlog indicates a delivery problem. | _gauge_     | `dataset`                                                                                                   |
...
| `embeddings_cache_evictions`<br/><br/>Number of entries removed from the cache, split by `reason`: `size` (over `max_size`), `expired` (past `item_ttl`), or `invalidated` (a refresh or DML write dropped the entries referencing a table). | _count_ | `reason` |
...
| `embeddings_cache_invalidation_stale_hits`<br/><br/>Exported for this cache but never incremented — only the SQL results cache serves results from invalidation-marked entries. Stays at the published zero; read `results_cache_invalidation_stale_hits` instead. | _count_ | — |
...
| `embeddings_cache_stale_rejections`<br/><br/>Exported for this cache but never incremented — only the SQL results cache records stale rejections. Stays at the published zero (one series per `reason`); read `results_cache_stale_rejections` instead. | _count_ | `reason` |
| `embeddings_cache_stale_swr_count`<br/><br/>Exported for this cache but never incremented — only the SQL results cache skips in-flight stale-while-revalidate refreshes. Stays at the published zero; read `results_cache_stale_swr_count` instead. | _count_ | — |
| `embeddings_cache_swr_background_query_count`<br/><br/>Exported for this cache but never incremented — only the SQL results cache triggers stale-while-revalidate background queries. Stays at the published zero; read `results_cache_swr_background_query_count` instead. | _count_ | — |
| `embeddings_cache_swr_revalidations`<br/><br/>Exported for this cache but never incremented — only the SQL results cache runs background stale-while-revalidate revalidations. Stays at the published zero (one series per `outcome`); read `results_cache_swr_revalidations` instead. | _count_ | `outcome` |
| `embeddings_cache_table_invalidations`<br/><br/>Exported for this cache but never incremented — only the SQL results cache applies table invalidations. Stays at the published zero (one series per `mode`); read `results_cache_table_invalidations` instead. | _count_ | `mode` |
...
| `http_requests`<br/><br/>Number of HTTP requests, counted when the response head is produced. A streaming response that fails after its head was sent is still counted here with the `200` status it started with — use `http_responses` to see the outcome.            | _count_     | `method`, `path`, `status`, _(request context)_                                                             |
| `http_requests_duration_ms`<br/><br/>Measures the duration of HTTP requests in milliseconds, up to the response head. For streaming responses (such as the default `/v1/sql` JSON format) the body continues after this is recorded — use `http_responses_duration_ms` for end-to-end duration. | _histogram_ | `method`, `path`, `status`, _(request context)_                                                             |
| `http_responses`<br/><br/>Number of HTTP responses, counted once the response body terminates. The `outcome` label reports how it terminated, so a streaming response that fails after its `200` head is not counted as a success.                                            | _count_     | `method`, `path`, `status`, `outcome` (one of `complete`, `error`, `incomplete`), _(request context)_        |
| `http_responses_duration_ms`<br/><br/>End-to-end HTTP response duration in milliseconds, from the request arriving to the response body terminating. Unlike `http_requests_duration_ms`, this includes the time spent streaming the body.                                | _histogram_ | `method`, `path`, `status`, `outcome` (one of `complete`, `error`, `incomplete`), _(request context)_        |
...
| `process_resident_anon_bytes`<br/><br/>Anonymous resident bytes of the `spiced` process: heap and stacks, which the kernel cannot reclaim. Emitted with `process_resident_memory_bytes`. | _gauge_     | —                                                                                                           |
| `process_resident_file_bytes`<br/><br/>File-backed resident bytes of the `spiced` process: mapped files and page cache the kernel evicts on demand. Emitted with `process_resident_memory_bytes`. | _gauge_     | —                                                                                                           |
| `process_resident_memory_bytes`<br/><br/>Total resident set size (RSS) of the `spiced` process, in bytes. Includes `process_resident_file_bytes`, so compare budgets and pool gauges against `process_resident_anon_bytes`. Sampled every 2 seconds. | _gauge_     | —                                                                                                           |
...
| `query_memory_pool_used_bytes`<br/><br/>Live bytes reserved in the query memory pool (`runtime.query.memory_limit`), excluding the in-memory CDC tier's mirror account. Sampled every 2 seconds, and only emitted when [Spice Cayenne](../components/data-accelerators/cayenne) acceleration is configured.                                                     | _gauge_     | —                                                                                                           |
...
| `results_cache_evictions`<br/><br/>Number of entries removed from the cache, split by `reason`: `size` (over `max_size`), `expired` (past `item_ttl`), or `invalidated` (a refresh or DML write dropped the entries referencing a table). | _count_ | `reason` |
...
| `results_cache_invalidation_stale_hits`<br/><br/>Number of results served from an entry a table invalidation had marked stale, within the configured stale-while-revalidate window. | _count_ | — |
...
| `results_cache_stale_rejections`<br/><br/>Number of lookups that found an entry but refused to serve it because a table the result read had since been invalidated. Split by `reason` into `no_window` (stale serving is disabled — `stale_while_revalidate_ttl` unset or `0s`), `window_expired` (the window had closed), or `fresh_required` (the lookup was made on a path that serves only fresh results). These are also counted as misses. | _count_ | `reason` |
...
| `results_cache_swr_revalidations`<br/><br/>Number of completed background stale-while-revalidate revalidations, split by `outcome` (`stored`, `query_failed`, `collect_failed`, `invalidated_mid_flight`, `transient_errors`, `unboundable`, `encode_failed`, `put_failed`). Any outcome other than `stored` leaves the previous entry in place to be served stale until it expires. | _count_ | `outcome` |
| `results_cache_table_invalidations`<br/><br/>Number of table invalidations applied to the cache, split by `mode` into `evict` (dependent entries removed) or `mark_stale` (left resident and marked stale, which `results_cache_evictions` cannot report). Counts invalidations, not the entries each one affected. | _count_ | `mode` |
...
| `schema_interner_rows`<br/><br/>Number of distinct Arrow schemas currently shared across cache entries. Populated by both the SQL results cache and the search results cache, so its bytes cannot be attributed to `results_cache_size_bytes` alone — read it against `search_results_cache_size_bytes` too. Shared schemas are not charged to any entry, so they appear in neither.                        | _gauge_     | —                                                                                                           |
| `schema_interner_value_bytes`<br/><br/>Total size of the shared Arrow schemas, counted once per distinct schema rather than once per cache entry holding it.                                                                  | _gauge_     | —                                                                                                           |
| `schema_interner_overhead_bytes`<br/><br/>The schema pool's own bookkeeping: its hash-map slots and bucket vectors.                                                                                                           | _gauge_     | —                                                                                                           |
| `schema_interner_collapsed`<br/><br/>Number of Arrow schemas collapsed onto an allocation the pool already held — the direct evidence that sharing is removing duplicates.                                                    | _count_     | —                                                                                                           |
| `schema_interner_already_shared`<br/><br/>Interns whose caller already held the shared schema allocation. Counted apart from `schema_interner_collapsed` because no duplicate was removed.                                    | _count_     | —                                                                                                           |
| `schema_interner_misses`<br/><br/>Interns that adopted an Arrow schema the pool had not seen. Rising misses mean schemas are arriving distinct and nothing is being shared only when `schema_interner_collapsed` **and** `schema_interner_already_shared` are both flat — a pool whose shapes settled long ago shares steadily through `already_shared` with `collapsed` flat.                                        | _count_     | —                                                                                                           |
| `search_results_cache_evictions`<br/><br/>Number of entries removed from the cache, split by `reason`: `size` (over `max_size`), `expired` (past `item_ttl`), or `invalidated` (a refresh or DML write dropped the entries referencing a table). | _count_ | `reason` |
...
| `search_results_cache_invalidation_stale_hits`<br/><br/>Exported for this cache but never incremented — only the SQL results cache serves results from invalidation-marked entries. Stays at the published zero; read `results_cache_invalidation_stale_hits` instead. | _count_ | — |
...
| `search_results_cache_stale_rejections`<br/><br/>Exported for this cache but never incremented — only the SQL results cache records stale rejections. Stays at the published zero (one series per `reason`); read `results_cache_stale_rejections` instead. | _count_ | `reason` |
| `search_results_cache_stale_swr_count`<br/><br/>Exported for this cache but never incremented — only the SQL results cache skips in-flight stale-while-revalidate refreshes. Stays at the published zero; read `results_cache_stale_swr_count` instead. | _count_ | — |
| `search_results_cache_swr_background_query_count`<br/><br/>Exported for this cache but never incremented — only the SQL results cache triggers stale-while-revalidate background queries. Stays at the published zero; read `results_cache_swr_background_query_count` instead. | _count_ | — |
| `search_results_cache_swr_revalidations`<br/><br/>Exported for this cache but never incremented — only the SQL results cache runs background stale-while-revalidate revalidations. Stays at the published zero (one series per `outcome`); read `results_cache_swr_revalidations` instead. | _count_ | `outcome` |
| `search_results_cache_table_invalidations`<br/><br/>Exported for this cache but never incremented — only the SQL results cache applies table invalidations. Stays at the published zero (one series per `mode`); read `results_cache_table_invalidations` instead. | _count_ | `mode` |
...
| `table_set_interner_rows`<br/><br/>Number of distinct input-table sets currently shared across cache entries. Populated by both the SQL results cache and the search results cache, so its bytes cannot be attributed to `results_cache_size_bytes` alone — read it against `search_results_cache_size_bytes` too. Shared table sets are not charged to any entry, so they appear in neither.  | _gauge_     | —                                                                                                           |
| `table_set_interner_value_bytes`<br/><br/>Total size of the shared input-table sets, counted once per distinct set rather than once per cache entry holding it.                                                               | _gauge_     | —                                                                                                           |
| `table_set_interner_overhead_bytes`<br/><br/>The table-set pool's own bookkeeping: its hash-map slots and bucket vectors.                                                                                                     | _gauge_     | —                                                                                                           |
| `table_set_interner_collapsed`<br/><br/>Number of input-table sets collapsed onto an allocation the pool already held — the direct evidence that sharing is removing duplicates.                                              | _count_     | —                                                                                                           |
| `table_set_interner_already_shared`<br/><br/>Interns whose caller already held the shared table-set allocation. Counted apart from `table_set_interner_collapsed` because no duplicate was removed.                           | _count_     | —                                                                                                           |
| `table_set_interner_misses`<br/><br/>Interns that adopted an input-table set the pool had not seen. Rising misses mean table sets are arriving distinct and nothing is being shared only when `table_set_interner_collapsed` **and** `table_set_interner_already_shared` are both flat — a pool whose shapes settled long ago shares steadily through `already_shared` with `collapsed` flat.                               | _count_     | —                                                                                                           |
...
| `tokio_runtime_worker_park_count`<br/><br/>Cumulative worker park count (summed across workers) per tokio runtime. Only emitted in builds compiled with the `tokio_unstable` cfg.                                              | _gauge_     | `runtime`                                                                                                   |
| `tokio_runtime_worker_steal_count`<br/><br/>Cumulative task-steal count (summed across workers) per tokio runtime. Only emitted in builds compiled with the `tokio_unstable` cfg.                                              | _gauge_     | `runtime`                                                                                                   |
| `tokio_runtime_workers`<br/><br/>Worker threads per tokio runtime.                                                                                                                                                            | _gauge_     | `runtime`                                                                                                   |
