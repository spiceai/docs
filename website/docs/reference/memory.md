...
- **Fail fast when the tier is saturated.** A retry that lands on another healthy instance helps. A retry against a tier that is already out of query slots, CPU, or memory (`503`, gRPC `RESOURCE_EXHAUSTED`, pool acquire timeouts on every client) adds load. While `query_failures{err_code="ResourcesExhausted"}` is sustained across instances, shed the call or fall back in the application, and turn limited retries back on only after those errors are rare and transient. See [Client connection pools](./performance-tuning#client-connection-pools).
