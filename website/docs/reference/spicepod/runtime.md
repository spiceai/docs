...
Spice supports adding optional authentication to its API endpoints via configurable API keys. [Learn more](/docs/api/auth).
...
The path to a temporary directory that Spice uses for query and acceleration operations that spill to disk. For more details, see the [Managing Memory Usage documentation](/docs/reference/memory) and the [DuckDB Data Accelerator documentation](/docs/components/data-accelerators/duckdb).
