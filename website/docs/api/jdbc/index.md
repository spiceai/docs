...
Size the JDBC pool as this application instance's share of the Spice tier's admission budget: the sum of [`max_concurrent_queries`](../reference/spicepod/runtime#runtimequerymax_concurrent_queries) across the Spice replicas, divided among every client instance that queries them. Validate the split under load. See [Client connection pools](../reference/performance-tuning#client-connection-pools). For short lookups over TLS Flight, see [Short queries](./arrow-flight-sql#short-queries).
