...
| `mysql_zero_date_behavior`| Controls how MySQL's `'0000-00-00'` / `'0000-00-00 00:00:00'` zero-date sentinel is handled for `DATE` / `DATETIME` / `TIMESTAMP` columns. `null` (default) coerces zero dates to `NULL` and widens affected columns to nullable in the Arrow schema. `error` fails the scan when a zero date is encountered and honors the source `NOT NULL` constraint exactly. |
