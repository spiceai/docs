...
The HTTP(s) Data Connector enables federated SQL query across [supported file formats](./index#file-formats) stored at an HTTP(s) endpoint. The connector supports dynamic query and data refresh through SQL-based filtering.
...
1. **Direct URL to a file**: A complete URL pointing to a specific [supported file](./index#file-formats).
