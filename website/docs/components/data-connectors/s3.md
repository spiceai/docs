...
Additionally, be aware that each query will retrieve data from the S3 source, which may result in significant network requests and bandwidth usage. It is important to consider the potential impact on network performance and costs associated with data transfer from S3.
