...
| `on_refresh_sort_columns`    | Columns to sort by during refresh. **Caution**: current implementation uses `CREATE OR REPLACE`, which drops `primary_key`, indexes, and `on_conflict` constraints. Prefer [Spice Cayenne](../cayenne/index.md) when you need clustering together with those constraints. |
...
- **`on_refresh_sort_columns` drops indexes**: The current implementation issues `CREATE OR REPLACE TABLE ... ORDER BY ...`, which drops pre-existing indexes, `primary_key`, and `on_conflict` constraints. Re-run `CREATE INDEX` statements after sort-column refreshes, or use [Spice Cayenne](../cayenne/index.md) (`sort_columns` / `cayenne_cluster_by`) when you need physical clustering together with those constraints.
...
| Indexes disappear after refresh                        | `on_refresh_sort_columns` triggers `CREATE OR REPLACE`.       | Re-create indexes post-refresh, avoid sort-column refreshes, or use [Spice Cayenne](../cayenne/index.md) for clustering with constraints.    |
