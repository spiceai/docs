...
| `cayenne_orphaned_dv_cleanup_min_files` | Number of orphan-eligible key-based deletion vectors that must accumulate before a background sweep reclaims their `.arrow` files. A key deletion vector becomes orphaned once time-based retention empties the protected snapshot(s) it shadowed. The sweep is lock-free and runs off the write path on the dedicated compaction runtime, so it only trades sweep frequency against lingering disk — never ingest latency. Applies to key-delete tables (`cayenne_deletion_mode: key`) only. Set to `0` (or leave unset) to disable cleanup entirely; a value ≥ `1` enables it. Defaults to `0` (disabled). |
