...
| `cayenne_file_pruning`            | Whether scans skip whole Vortex files using file-level statistics (min/max) and partition values before opening them. Accepts `enabled` (default), `disabled`, `true`, or `false`. `enabled` prunes non-matching files without reading them; `disabled` opens and scans every candidate file. Invalid values fall back to `enabled`. |
