...
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `auth_method`   | Optional. One of `service_principal`, `managed_identity`, `workload_identity`, `cli`, or `default`. Defaults to `default`, which auto-detects credentials via the [DefaultAzureCredential](https://learn.microsoft.com/dotnet/api/azure.identity.defaultazurecredential) chain. |
| `tenant_id`     | Required for `auth_method: service_principal`. The Microsoft Entra tenant ID.                                                                                                                                                                                                   |
| `client_id`     | Required for `auth_method: service_principal` and `auth_method: workload_identity`. The application (client) ID. Optional for `managed_identity` to select a user-assigned identity.                                                                                            |
...
- **`default`** (the default) — uses the [DefaultAzureCredential](https://learn.microsoft.com/dotnet/api/azure.identity.defaultazurecredential) chain, attempting environment variables, workload identity, managed identity, and the Azure CLI in order. Suitable for most deployments.
...
- **`workload_identity`** — uses [federated tokens](https://learn.microsoft.com/azure/aks/workload-identity-overview) injected by AKS or another Kubernetes cluster configured with workload identity. Requires `client_id` and `tenant_id` (typically supplied by the workload identity admission webhook through `AZURE_CLIENT_ID` / `AZURE_TENANT_ID`).
