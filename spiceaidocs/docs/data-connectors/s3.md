...
- `has_header`: CSV only. Indicate if the CSV file has header row. default: `true`
- `quote`: CSV only. A one-character string used to quote fields containing special characters. default: `"`
- `escape`: CSV only. A one-character string used to represent special characters or to include characters that would normally be interpreted as delimiters or new line characters within a field value. default: null
- `schema_infer_max_records`: CSV only. A number used to set the limit in terms of records to scan to infer the schema. default `1000`
- `delimiter`: CSV only. A one-character string used to separate individual fields. default: `,`
