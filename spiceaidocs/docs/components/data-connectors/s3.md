...
For endpoints protected by access keys, `key` and `secret` are required and must be passed using a [Secrets Store](/components/secret-stores) or via `spice s3 login`.
